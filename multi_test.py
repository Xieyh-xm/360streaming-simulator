...
def test(net_id, video_id, user_id, avgs_q):
    ...
    # print_metrics(avgs)
    print_to_csv(csv_file_path, net_id, video_id, user_id, avgs)
    avgs_q.put(avgs)
    # return avgs
...
def test_user_samples(net_id, video_id, user_batch=USER_TRACE_NUM):
    ...
    avgs_q = multiprocessing.Queue()
    test_jobs = []

    for user_id in random.sample(user_list, user_batch):
        p = multiprocessing.Process(target=test, args=(net_id, video_id, user_id, avgs_q))
        test_jobs.append(p)
        p.start()
        # avgs += test(net_id, video_id, user_id, avgs_q)

    for job in test_jobs:
        job.join()  # 等待子进程结束后再继续往下运行
