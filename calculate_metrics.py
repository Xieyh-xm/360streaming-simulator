...
movie_file = 'video/manifest/movie360.json'
default_log_file = 'log/session.log'
...
re_download_complete = re.compile(
    '.* download complete: segment:([0-9]+) tile:([0-9]+) quality:([0-9]+) ([0-9]+)/([0-9]+)bits')
re_download_abandoned = re.compile(
    '.* download abandoned: segment:([0-9]+) tile:([0-9]+) quality:([0-9]+) ([0-9]+)/([0-9]+)bits')
...
def calculate_metrics_up_to(time):
    ...
    if visible_tiles > 0:
        ...
        average_utility += ((utility - blank_penalty * blank_tiles) / (visible_tiles + blank_tiles) - average_utility) \
                           * period / play_period
        average_blank += (blank_tiles / (blank_tiles + visible_tiles) - average_blank) * period / (
                play_period + stall_period)
        if blank_tiles > 0:
...
if __name__ == '__main__':
    ...
    output.add('complete_play_period', '%.3f' % ((play_period - incomplete_play_period) / 1000), 's',
               'all tiles rendering')
    output.add('incomplete_play_period', '%.3f' % (incomplete_play_period / 1000), 's',
               'at least one tile rendering, at least one tile blank')
    ...
    output.add('complete_ratio', '%.6f' % ((play_period - incomplete_play_period) / (play_period + stall_period)), '',
               'complete_play_period / video_length')
    output.add('incomplete_ratio', '%.6f' % (incomplete_play_period / (play_period + stall_period)), '',
               'incomplete_play_period / video_length')
    output.add('stall_ratio', '%.6f' % (stall_period / (play_period + stall_period)), '', 'stall_ratio / video_length')
    output.add('average_quality', '%.3f' % average_quality, '',
               'first average over visible tiles, then over video; blank tiles give -1 quality')
    output.add('average_bitrate', '%.0f' % average_bitrate, 'kbit/s', 'blank tiles give 0 bitrate')
    output.add('average_utility', '%.3f' % average_utility, '',
               'blank tiles give utility[max_bitrate]=%.3f penalty' % blank_penalty)
    ...
    output.add('total_rendered_bits', '%.3f' % (total_rendered_bits / 1000000), 'Mbit',
               'downloaded tiles that were used')
    ...
    output.add('abandoned_bits', '%.3f' % (abandoned_bits / 1000000), 'Mbit',
               'download requests abandoned before completion')
